...
- [x] SelectViewがHandlebarsテンプレートを使用している
- [x] セマンティックなHTML構造になっている
- [x] テンプレート修正でホットリロードが動作する
- [x] 既存のテストが全てパスする
- [x] 新しいテスト（セマンティック構造、アクセシビリティ）がパスする

### フェーズ2完了時

- [x] ResultViewがHandlebarsテンプレートを使用している
- [x] SelectView → ゲーム → ResultView の流れが正常に動作する
- [x] コードの重複が削減されている
